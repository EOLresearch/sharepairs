...
export default function UserCard({
      ...
      className={
        selectedUser
          ? selectedUser.uid === user.uid
            ? hovered === true
              ? 'user-card user-selected hovered'
      ...
      <div className="card-header">
        ...
        <div className={user.simpaticoMatch ? 'matched' : 'not-matched'}>
          <span className="card-label">
            {user.simpaticoMatch ? 'Matched' : 'Not Matched'}
          </span>
        </div>
      </div>
      ...
      <div className="card-content">
        ...
        <div className="card-row">
          ...
          <span>
            &nbsp;
            {user.simpaticoMatch
              ? user.simpaticoMatch.displayName
              : 'user not matched'}
          </span>
        </div>
        <div className="card-row">
        ...
        <div className="card-btn-container">
          {user.simpaticoMatch ? (
            ...
            <button
              onClick={matchInputDisplaySwitch}
              className="card-btn"
            >
              Matching options
            </button>
          ...
          <button
            className="card-btn"
            onClick={showDetailsDisplaySwitch}
          >
            Show Details
          </button>
        </div>
        ...
        {showMatchingOptions === true ? (
            ...
            <div className="matching-btns-container">
              <div className="manual-matching-options">
                ...
                >
                  ...
                  {!selectedUser || selectedUser.uid === user.uid
                    ? 'Select a user'
                    : `Match with: ${selectedUser.displayName}`}
                </button>
              </div>

              {toolTip ? (
                <h5 className="tool-tip">{toolTip}</h5>
              ) : (
              ...
              <div className="quick-matching-options">
                ...
                <button
                  data-tooltip="both"
                  onMouseEnter={(e) =>
                    setToolTip('Match by KINSHIP & CAUSE')
                  }
                  onMouseLeave={(e) => setToolTip('')}
                  onClick={(e) => matchConfirm(e)}
                >
          ...
          <div className="match-container">
            <span className="match-confirm-message">
              Match this user with{' '}
              <strong>
                {selectedUser ? selectedUser.displayName : 'No Selected User'}
              </strong>{' '}
              ?
            </span>
            <div className="match-confirm-btns">
              <button
                className="match-button"
                onClick={(e) => matchDoubleConfirm(e, user.uid)}
              >
                Confirm
              </button>
              <button
                className="match-button"
                onClick={(e) => cancelMatchConfirm(e)}
              >
                Cancel
              </button>
            </div>
          ...
          <div className="match-container">
            <div className="matching-btns-container">
              ...
              {toolTip ? (
                ...
                <h5>Random Matching options</h5>
              )}
              <div className="quick-matching-options">
                <button
                  data-tooltip="Match by CAUSE only"
                  onMouseEnter={(e) => setToolTip('Match by CAUSE')}
                  onMouseLeave={(e) => setToolTip('')}
                >
                  C
                ...
                <button
                  data-tooltip="Match by KINSHIP only"
                  onMouseEnter={(e) => setToolTip('Match by KINSHIP')}
                  onMouseLeave={(e) => setToolTip('')}
                >
                  K
                ...
                <button
                  data-tooltip="Match by KINSHIP and CAUSE"
                  onMouseEnter={(e) =>
                    setToolTip('Match by KINSHIP & CAUSE')
                  }
                  onMouseLeave={(e) => setToolTip('')}
                ...
                <button
                  data-tooltip="Match by NO MATCH"
                  onMouseEnter={(e) => setToolTip('Match by NO MATCH')}
                  onMouseLeave={(e) => setToolTip('')}
                >
                  X
  ...
  return <div>{showDetails ? expanded : collapsed}</div>;
}
